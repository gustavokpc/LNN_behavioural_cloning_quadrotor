...
start_pos = gate_pos[0] + np.array([0, 0, 0])
...
class Quadcopter3DGates(VecEnv):
    def __init__(self,
        ...
        self.low_obs_state_len = self.state_len - 6  # remove vel and rates
    ...
    def reset_(self, dones):
        ...
        if self.initialize_at_random_gates:
            ...
            # x0 = np.random.uniform(-3, 3, size=(num_reset,)) + start_pos[0]
            # y0 = np.random.uniform(-4, 4, size=(num_reset,)) + start_pos[1]
            # z0 = np.random.uniform(0, 4, size=(num_reset,)) + start_pos[2]
            ...
            z0 = np.random.uniform(-3, 0, size=(num_reset,))
        ...
        # vx0 = np.random.uniform(-2.5, 2.5, size=(num_reset,))
        # vy0 = np.random.uniform(-2.5, 2.5, size=(num_reset,))
        # vz0 = np.random.uniform(-2.5, 2.5, size=(num_reset,))

        vx0 = np.random.uniform(-0.5, 0.5, size=(num_reset,)) / 5
        vy0 = np.random.uniform(-0.5, 0.5, size=(num_reset,)) / 5
        vz0 = np.random.uniform(-0.5, 0.5, size=(num_reset,)) / 5

        phi0 = np.random.uniform(-np.pi / 9, np.pi / 9, size=(num_reset,)) / 4
        theta0 = np.random.uniform(-np.pi / 9, np.pi / 9, size=(num_reset,)) / 4
        psi0 = np.random.uniform(-np.pi, np.pi, size=(num_reset,))

        p0 = np.random.uniform(-0.1, 0.1, size=(num_reset,)) / 5
        q0 = np.random.uniform(-0.1, 0.1, size=(num_reset,)) / 5
        r0 = np.random.uniform(-0.1, 0.1, size=(num_reset,)) / 5

        w10 = np.random.uniform(-1, 1, size=(num_reset,)) / 2
        w20 = np.random.uniform(-1, 1, size=(num_reset,)) / 2
        w30 = np.random.uniform(-1, 1, size=(num_reset,)) / 2
        w40 = np.random.uniform(-1, 1, size=(num_reset,)) / 2
    ...
    def step_wait(self):
        ...
        for i in range(self.num_envs):
            if dones[i]:
                ...
                # print("Done by:")
                # if ground_collision[i]:
                #     print("Ground_collision")
                # elif out_of_bounds[i]:
                #     print("Out of bounds")
                # elif gate_collision[i]:
                #     print("Gate collision")
                # elif max_steps_reached[i]:
...
parallel_env = Quadcopter3DGates(
    num_envs=100,
    gates_pos=gate_pos,
    gate_yaw=gate_yaw,
    start_pos=start_pos,
...
parallel_env_low_obs = Quadcopter3DGates(
    num_envs=100,
    gates_pos=gate_pos,
    gate_yaw=gate_yaw,
    start_pos=start_pos,
...
test_env_low_obs = Quadcopter3DGates(
    num_envs=1,
    gates_pos=gate_pos,
    gate_yaw=gate_yaw,
    start_pos=start_pos,

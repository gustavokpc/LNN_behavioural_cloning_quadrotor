...
void nn_control(const float *state, float *control) {
    ...
    float hnew[HIDDEN_SIZE], cnew[HIDDEN_SIZE];
    for (int j=0;j<HIDDEN_SIZE;++j) {
        float gates[4];
        for (int g=0; g<4; ++g) {
            int row=g*HIDDEN_SIZE+j;
            float acc=rnn_lstm_bias_ih_l0[row]+rnn_lstm_bias_hh_l0[row];
            for (int i=0;i<CONV_FEATURES;++i) acc += rnn_lstm_weight_ih_l0[row*CONV_FEATURES+i]*feat[i];
            for (int i=0;i<HIDDEN_SIZE;++i) acc += rnn_lstm_weight_hh_l0[row*HIDDEN_SIZE+i]*nn_hidden[i];
            gates[g]=acc;
        }
        float ing=sigmoidf_local(gates[0]); float forget=sigmoidf_local(gates[1]); float cellg=tanhf(gates[2]); float outg=sigmoidf_local(gates[3]);
        cnew[j]=forget*nn_cell[j]+ing*cellg; hnew[j]=outg*tanhf(cnew[j]);
    }
    for (int j=0;j<HIDDEN_SIZE;++j) { nn_hidden[j]=hnew[j]; nn_cell[j]=cnew[j]; }

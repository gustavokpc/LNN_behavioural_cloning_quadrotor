...
void nn_reset(void) { for (int i=0;i<HIDDEN_SIZE;++i) { nn_hidden[i]=0.0f; nn_cell[i]=0.0f; } }
void nn_control(const float *state, float *control) {
    float x[NUM_STATES], feat[CONV_FEATURES];
    normalize(state, x);
    conv_features(x, feat);

    float r[HIDDEN_SIZE], z[HIDDEN_SIZE], n[HIDDEN_SIZE], hnew[HIDDEN_SIZE];
    for (int j=0;j<HIDDEN_SIZE;++j) {
        float xir=rnn_gru_bias_ih_l0[j], xhr=rnn_gru_bias_hh_l0[j];
        float xiz=rnn_gru_bias_ih_l0[HIDDEN_SIZE+j], xhz=rnn_gru_bias_hh_l0[HIDDEN_SIZE+j];
        float xin=rnn_gru_bias_ih_l0[2*HIDDEN_SIZE+j], xhn=rnn_gru_bias_hh_l0[2*HIDDEN_SIZE+j];
        for (int i=0;i<CONV_FEATURES;++i) { xir += rnn_gru_weight_ih_l0[j*CONV_FEATURES+i]*feat[i]; xiz += rnn_gru_weight_ih_l0[(HIDDEN_SIZE+j)*CONV_FEATURES+i]*feat[i]; xin += rnn_gru_weight_ih_l0[(2*HIDDEN_SIZE+j)*CONV_FEATURES+i]*feat[i]; }
        for (int i=0;i<HIDDEN_SIZE;++i) { xhr += rnn_gru_weight_hh_l0[j*HIDDEN_SIZE+i]*nn_hidden[i]; xhz += rnn_gru_weight_hh_l0[(HIDDEN_SIZE+j)*HIDDEN_SIZE+i]*nn_hidden[i]; xhn += rnn_gru_weight_hh_l0[(2*HIDDEN_SIZE+j)*HIDDEN_SIZE+i]*nn_hidden[i]; }
        r[j]=sigmoidf_local(xir+xhr); z[j]=sigmoidf_local(xiz+xhz); n[j]=tanhf(xin + r[j]*xhn); hnew[j]=n[j] + z[j]*(nn_hidden[j]-n[j]);
    }
    for (int j=0;j<HIDDEN_SIZE;++j) nn_hidden[j]=hnew[j];

...
def compile_lib() -> Path:
    ...
    subprocess.run(
        [
            ...
            str(C_DIR / "nn_operations.c"), str(C_DIR / "nn_parameters.c"), "-lm", "-o", str(lib),
        ],
        check=True,
    )
    return lib
...
def main() -> None:
    ...
    lib.nn_control.argtypes = [ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float)]
    ...
    for idx, raw_state in enumerate(states):
        ...
        c_out = arr4()
        lib.nn_control(arr19(*raw_state.tolist()), c_out)

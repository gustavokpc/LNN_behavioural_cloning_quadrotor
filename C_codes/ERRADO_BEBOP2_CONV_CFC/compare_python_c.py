...
CONFIG_PATH = PROJECT / "configs" / "ERRADO_new_conv_bebop2_CFC_64_neurons_epoch=18_val_loss=0.000127_cfc_compat.yaml"
NORMALIZATION_MODULE = "LNN_behavioural_cloning_quadrotor.utils.normalization_limits_bebop2_new"
DT = 0.01
STATE = np.array(
...
def py_out() -> np.ndarray:
    ...
    norm = importlib.import_module(NORMALIZATION_MODULE)
    norm_min = np.asarray([norm.GLOBAL_MIN["omega_min"] if label.startswith("omega") else norm.GLOBAL_MIN[label] for label in labels], dtype=np.float32)
    norm_max = np.asarray([norm.GLOBAL_MAX["omega_max"] if label.startswith("omega") else norm.GLOBAL_MAX[label] for label in labels], dtype=np.float32)

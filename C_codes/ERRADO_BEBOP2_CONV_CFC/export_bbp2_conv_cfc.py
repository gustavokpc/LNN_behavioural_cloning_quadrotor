#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Regenerate the BBP2 Conv-CfC C parameter export from a PyTorch checkpoint."""
...
def _load_norm_limits(module_name: str) -> tuple[dict[str, float], dict[str, float]]:
    module = importlib.import_module(module_name)
    return module.GLOBAL_MIN, module.GLOBAL_MAX
...
def write_parameters(checkpoint_path: Path, config_path: Path, output_dir: Path, normalization_module: str) -> None:
    config = load_yaml(config_path)
    state_dict = _load_state_dict(checkpoint_path)
    input_order = _input_order(config)
    norm_min_by_name, norm_max_by_name = _load_norm_limits(normalization_module)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=PROJECT / "checkpoints" / "bbp2_conv_cfc_default_n64_epoch=19_val_loss=0.000143.ckpt",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT / "configs" / "bbp2_conv_cfc_default_n64_epoch=19_val_loss=0.000143.yaml",
    )
    parser.add_argument("--output-dir", type=Path, default=C_DIR)
    parser.add_argument(
        "--normalization-module",
        default="LNN_behavioural_cloning_quadrotor.utils.normalization_limits_bebop2",
        help="Python module containing GLOBAL_MIN/GLOBAL_MAX for C-export input normalization.",
    )
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    write_parameters(args.checkpoint, args.config, args.output_dir, args.normalization_module)

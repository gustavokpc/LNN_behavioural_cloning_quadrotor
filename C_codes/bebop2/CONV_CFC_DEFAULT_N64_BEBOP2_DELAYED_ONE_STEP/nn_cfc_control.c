 ...
 * Set NN_CFC_RATE_FILTER_ENABLED to 0 to compile it out completely.
 */
...
static struct FirstOrderLowPass nn_cfc_rate_filter_p;
static struct FirstOrderLowPass nn_cfc_rate_filter_q;
static struct FirstOrderLowPass nn_cfc_rate_filter_r;
...
static void nn_cfc_filter_rate_inputs(float state[NUM_STATES])
{
  if (!nn_cfc_rate_filters_initialized) {
    ...
    init_first_order_low_pass(
        &nn_cfc_rate_filter_p, tau, NN_CFC_RATE_FILTER_SAMPLE_TIME_S, state[9]);
    init_first_order_low_pass(
        &nn_cfc_rate_filter_q, tau, NN_CFC_RATE_FILTER_SAMPLE_TIME_S, state[10]);
    init_first_order_low_pass(
        &nn_cfc_rate_filter_r, tau, NN_CFC_RATE_FILTER_SAMPLE_TIME_S, state[11]);
    nn_cfc_rate_filters_initialized = true;
    return;
  }

  state[9] = update_first_order_low_pass(&nn_cfc_rate_filter_p, state[9]);
  state[10] = update_first_order_low_pass(&nn_cfc_rate_filter_q, state[10]);
  state[11] = update_first_order_low_pass(&nn_cfc_rate_filter_r, state[11]);
}
#else
static void nn_cfc_reset_rate_filters(void)
{

#!/usr/bin/env python3
"""Export conv_cfc_default_n64_bebop2_delayed_correct_sign for Paparazzi."""
...
CONFIG = (
    PROJECT
    / "configs/bebop2/conv_cfc_default_n64_bebop2_delayed_correct_sign.yaml"
)
...
def main() -> None:
    ...
    model = config["model"]
    if "dt" in dataset["input_labels"]:
        raise ValueError("Expected a checkpoint trained without dt.")
    ...
    if not dataset.get("with_delay", False) or dataset.get("delay_steps") != 1:
        raise ValueError("Expected a checkpoint trained with a one-step input delay.")

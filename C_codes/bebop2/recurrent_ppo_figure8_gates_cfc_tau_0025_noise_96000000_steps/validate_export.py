...
def python_actor_step(
    ...
    return torch.clamp(raw, 0.0, 1.0), hidden, raw


def main() -> None:
    library = Path("/tmp/librl_cfc_tau_0025_noise_96000000.so")
    subprocess.run(
        [
            "gcc",
            "-std=c99",
    ...
    rng = np.random.default_rng(96_000_025)
    observations = rng.normal(0.0, 1.0, size=(500, 20)).astype(np.float32)
    observations[:, 12:16] = rng.uniform(0.0, 1.0, size=(500, 4))
    ...
    for observation in observations:
        ...
        hidden_c_array = np.asarray(hidden_c, dtype=np.float32)
        raw_c_array = np.asarray(raw_c, dtype=np.float32)
        max_action_error = max(
            max_action_error, float(np.max(np.abs(action_c - action_py.numpy())))
        )
        max_hidden_error = max(
            max_hidden_error, float(np.max(np.abs(hidden_c_array - hidden_py.numpy())))
        )
        max_raw_error = max(
            max_raw_error, float(np.max(np.abs(raw_c_array - raw_py.numpy())))
        )

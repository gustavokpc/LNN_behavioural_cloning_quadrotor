...
#define EMBEDDED_CFC_TIMESPAN 0.01f
static float nn_hidden[HIDDEN_SIZE];
static float nn_timespan = EMBEDDED_CFC_TIMESPAN;
...
void nn_set_timespan(float timespan) {
    nn_timespan = timespan > 0.0f ? timespan : EMBEDDED_CFC_TIMESPAN;
}
int nn_hidden_size(void) { return HIDDEN_SIZE; }
void nn_get_hidden(float *hidden_out) {
    for (int i=0;i<HIDDEN_SIZE;++i) hidden_out[i]=nn_hidden[i];
}
void nn_control(const float *state, float *control) {
    ...
    for (int i=0;i<128;++i) bb[i]=reluf_local(bb[i]);
    ...
    float timespan_scale = nn_timespan / EMBEDDED_CFC_TIMESPAN;
    for (int i=0;i<HIDDEN_SIZE;++i) { float f1=tanhf(ff1[i]); float f2=tanhf(ff2[i]); float t=sigmoidf_local(ta[i]*timespan_scale+tb[i]); nn_hidden[i]=f1*(1.0f-t)+t*f2; }

...
CHECKPOINT = (
    PROJECT
    / "rl/checkpoints/CFC/recurrent_ppo_with_plus1/"
    "recurrent_ppo_figure8_gates_ckpt_44800000_steps.zip"
)
...
def main() -> None:
    ...
    if metadata.get("num_timesteps") != 44_800_000:
        raise ValueError(f"Unexpected checkpoint step: {metadata.get('num_timesteps')!r}")
    if policy_kwargs.get("cfc_timespan") != 0.01:
        raise ValueError(f"Unexpected cfc_timespan: {policy_kwargs.get('cfc_timespan')!r}")
    ...
    header_lines = [
        ...
        " * recurrent_ppo_figure8_gates_ckpt_44800000_steps.zip",
        " * from recurrent_ppo_with_plus1, configured for a 100 Hz controller",
        " * (CfC timespan = 0.01 s, trained action range = [0, 1]).",
        " */",
    ...
    print(f"Exported exact 44,800,000-step actor to {HERE}")

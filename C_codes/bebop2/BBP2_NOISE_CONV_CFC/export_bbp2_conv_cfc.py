...
def _norm_vectors(input_order: list[str]) -> tuple[np.ndarray, np.ndarray]:
    mins, maxs = get_norm_vectors(input_order, "bebop2_tau_0_06")
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=PROJECT / "checkpoints" / "bebop2" / "BBP2_NOISE_conv_cfc_default_n64_epoch=19_val_loss=0.000142.ckpt",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT / "configs" / "bebop2" / "BBP2_NOISE_conv_cfc_default_n64_epoch=19_val_loss=0.000142.yaml",
    )
